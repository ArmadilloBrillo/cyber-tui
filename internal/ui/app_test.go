...
func TestNavigateTab_LeftFromFeed_Wraps(t *testing.T) {
	...
	if a.active != screenProfile {
		t.Errorf("expected screenProfile (wrap), got %v", a.active)
	}
}
...
func TestNavigateTab_RightFromProfile_Wraps(t *testing.T) {
	...
	if a.active != screenFeed {
		t.Errorf("expected screenFeed (wrap), got %v", a.active)
	}
}
...
func TestNavigateTab_CyclesAllTabsRight(t *testing.T) {
	a := loggedInApp()
	a.active = screenFeed
	expected := []screen{screenChatrooms, screenDMs, screenProfile, screenFeed}
...
func TestNavigateTab_CyclesAllTabsLeft(t *testing.T) {
	a := loggedInApp()
	a.active = screenFeed
	expected := []screen{screenProfile, screenDMs, screenChatrooms, screenFeed}
	for i, want := range expected {
		a.navigateTab(-1)
		if a.active != want {
			t.Errorf("step %d: expected %v, got %v", i+1, want, a.active)
