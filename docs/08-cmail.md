...
Like CIRC (`docs/33-circ.md`), any `/`-prefixed input not recognized is rejected client-side on Enter — the input clears and a local `*** unknown command: /foo` system notice appears instead of the text being sent as a literal message. This uses the same `isKnownSlashCommand` helper CIRC uses (`chatrooms.go`), since the server accepts nearly the same command set for both: `/me`, the emotes, `/dice`, `/8ball`, `/fortune`, `/gif`, `/song`, `/help`, `/bork`, and chained text styles (`/comic+rainbow`, etc. — same `/spoiler`-can't-chain rule as CIRC). Only `/mute`/`/unmute`/`/muted`/`/unmuteall` and `/art` are CIRC-only and rejected in C-Mail.

`/dice` accepts full notation the same as CIRC — bare `/dice`, `/dice:SIDES`, `/dice:SIDES:COUNT` (colon shorthand, e.g. `/dice:20:2`), or free-form space-separated notation (`/dice 4d6kh3`, `/dice adv`, `/dice d%`, etc.). The colon form needed a `strings.HasPrefix(cmd, "/dice:")` carve-out in `isKnownSlashCommand` to be recognized at all, since it has no space before its digits and so arrives as one whole token that would never exact-match the bare `/dice` entry in `baseSlashCommands` — see `docs/33-circ.md`'s "Slash commands" section for the full explanation (shared code, one fix covers both screens). As with every other command, only the name is checked here; bad notation still 400s server-side.

`/bork <text>` works the same as in CIRC: the text is rewritten locally and sent as a plain message, and the `/help` reply gains a `/bork` line. See `docs/57-bork.md`.
