...
- **Message viewport**: scrollable history in IRC-style format.
  ...
  - **Inline markdown formatting**: message bodies support `*emphasis*`/`_italics_`, `**bold**`, `` `code` ``, `[links](url)`, and bare-URL auto-linking, via `markdown.RenderInline` (`internal/ui/markdown/renderer.go`). Deliberately *not* full `markdown.Render` (used by Feed/PostDetail/Journal/Guilds/Topics/C-Mail): that does GFM block-level parsing, which would reflow multi-line content (a literal `\n` inside a paragraph becomes a soft-break space, collapsing intentional line breaks — e.g. a multi-line `/fortune` reply) and would misinterpret a plain chat line that happens to start with `#`, `-`, `>`, `` ``` ``, or `---` as a heading/list/blockquote/code-fence/thematic-break. `RenderInline` instead uses a goldmark parser instance registering *only* a paragraph block parser (no heading/list/blockquote/fenced-code/thematic-break/html-block parsers at all — confirmed via `parser.NewParser()` starting with zero registered parsers and `WithBlockParsers`/`WithInlineParsers` only appending to that), with the normal CommonMark inline set (`parser.DefaultInlineParsers()`: code span, link, autolink, raw HTML, emphasis) plus bare-URL detection (`extension.NewLinkifyParser()`, the same one `extension.GFM`'s Linkify wraps) — and parses each line of a message independently, rejoining with `\n` verbatim, so goldmark's soft-line-break-to-space behavior (which only triggers when multiple lines reach the parser as one paragraph) never applies. No `@mention` AST node — CIRC's own bespoke ghost-text mention-completion system (`chatrooms.go`) is unaffected, layered on top separately. Own-username highlighting (bolding a mention of the viewer's own name in `theme.MeHighlight`) is a `RenderInline` parameter (`highlightUser`), applied at the same raw-text level as the rest of the styling — a prior version did this as a separate post-render pass (`highlightMentions`, a regex splicing a fresh `Render()` call into already-ANSI-rendered text), which broke surrounding styling for everything after the match (an inner `Render()` call's own SGR reset clears *all* active attributes, not just what it set) — e.g. `@ragnar 1` rendered `1` correctly but `@ragnar 1 2` left `1` unstyled and only `2` back in `theme.Base`'s color. Fixed by moving the highlighting into `RenderInline` itself, built from plain text once rather than re-rendering an already-styled string; the trade-off is a mention of your own name *inside* emphasis/bold/code (e.g. `**@ragnar**`) no longer gets the special override (a rare edge case, and the old regex-based approach only caught it by accident). `renderActionLine`'s outer `"* username body *"` `/me` wrapping is plain string concatenation applied *after* the body is rendered, so it's never re-parsed as markdown — a body containing its own literal `*` characters doesn't confuse it (goldmark's emphasis parser also falls back to literal `*` on unmatched delimiters, standard CommonMark behavior).
- **Input box** (3 rows, at the bottom): type and press `Enter` to send. `input.Width` is set to `terminalWidth - 4 - len(Prompt) - 1`, not just `terminalWidth - 4` — `bubbles/textinput`'s empty-placeholder render and its typed-content render total different widths (typed content adds `Prompt`'s width plus one more for the phantom end-of-line cursor glyph, neither subtracted from its own padding math), so without that compensation the box silently rendered 3 columns wider than the header above it the instant any character was typed, pushing its right border off-screen on a narrow-enough terminal. The empty (placeholder) state itself is also hand-built rather than left to `textinput.View()`'s own `placeholderView()` — that internal path totals its render at exactly `Width`, 3 columns short of the (correct) typed-content total, so relying on it left the box undersized until the first keystroke. Both states are now constructed directly from `textinput`'s exported style fields (`PromptStyle`, `PlaceholderStyle`/`TextStyle`, `Cursor`) so the total is guaranteed by construction instead of depending on matching an internal library formula. Same fix applied to C-Mail's compose input (`docs/08-cmail.md`).
...
- **`@mention` completion**: typing `@` starts an in-progress mention token (start-of-input or preceded by whitespace, so `user@host` never triggers). `Tab` previews an online user whose name matches the typed prefix (`@` alone previews everyone online, in the panel's admins-first/alphabetical order) as dim ghost text — nothing is inserted into the real message yet, and the cursor never moves. The cursor itself overlays the ghost's first character rather than sitting in its own blank cell, so there's no gap between what you typed and the preview — and that overlaid character is always styled in the same dim ghost color (`cursor.Model.TextStyle`, set to `theme.Subtle`), so it never reads as something you actually typed. Before any `Tab` press the first match previews by default; the first `Tab` skips straight to the *second* match (the first was already showing, so starting a cycle there would look like a no-op), and each further `Tab` cycles to the next, wrapping around. Typing a **space** commits whatever's currently previewed as real text, then inserts the space after it; typing anything else instead just clears the preview, leaving exactly what you actually typed untouched. Matches come from `m.roomUsers` (the same live presence list backing the users panel), so only people currently online are offered — and re-resolved fresh on every `Tab` press and every render (`mentionActiveCandidate`), not snapshotted when cycling starts, so someone leaving mid-cycle stops being offered and someone joining becomes reachable, both without needing to retype the query to start a fresh cycle. `mentionCycle` only stores a Tab-press counter, wrapped against the live match count wherever it's read — one accepted trade-off: if the roster's order or membership changes between two `Tab` presses, the *position* being cycled can land on a different person than the one shown a moment ago (favored over the alternative of offering someone who already left). The preview only renders when the mention token reaches the very end of the input — with trailing text after it (`hey @al there`) there's no unambiguous place to draw it, so `Tab`/space still work there, just without the visual hint. Rendering the cursor-overlaid preview bypasses `textinput.View()` (there's no way to make it overlay external text through that API) and hand-builds the line from its own exported style fields instead — this means it doesn't get `textinput`'s internal horizontal-scroll windowing, so a mention typed after a message has grown long enough to need horizontal scrolling won't scroll correctly; mentions are almost always typed early in short messages, so this hasn't been worth fixing. See `mentionCycle`, `mentionQueryAt`/`matchMentionCandidates`/`spliceMention`, and `ChatroomsModel.mentionGhostText` in `chatrooms.go`.
...
The presence subscription (`SubscribeRoomPresence`) reconnects the same way — `attemptReconnect` (`reconnect.go`) is shared, generic over the streamed value type — but tracks its retry state separately (`presenceReconnect*` fields) since the message and presence streams close/reconnect independently. It also passes the last known-good `roomUsers` list as `initial` on every (re)subscribe, seeding the merge state so the panel doesn't render empty until the first fresh snapshot arrives.
...
- **Presence stream reconnect has no UI indicator**: unlike the message stream, a dropped `chat_presence` connection retries silently — nothing shown to the user if it keeps failing (it's supplementary data, not core chat functionality). It does now share the message stream's `RefreshSession` + exponential-backoff reconnect machinery (`reconnect.go`, `attemptReconnect`), tracked separately (`presenceReconnect*` fields on `ChatroomsModel`) since the two streams can each independently close and reconnect — this closed a real bug where the presence stream, lacking `RefreshSession`, could spin retrying against an already-expired token until the message stream's own reconnect happened to refresh it as a side effect.
- **Presence resubscribe is seeded from the last known-good list**: `SubscribeRoomPresence`'s internal merge state used to start empty on every (re)subscribe, and the channel only emits after the first fresh event/patch or the 5s re-evaluation tick arrives — so a resubscribe (reconnect, or the quick leave/re-enter race below) could render the panel as briefly empty, with real users trickling back in one at a time as their own heartbeats re-announced them. Reconnects now pass the last known-good `roomUsers` list as `initial`, which seeds the merge state and is delivered immediately (before waiting on any event), closing that gap.
- **Stale/duplicate presence subscriptions are guarded against**: a quick leave/re-enter of the same room slug could previously leave an orphaned, uncancelled `SubscribeRoomPresence` subscription running (its `announcePresenceCmd` result isn't cancellable), whose snapshots could still land and clobber the active list since `roomPresenceReceivedMsg` carried no way to tell which subscription produced it. It now carries the originating subscription and is ignored unless it matches the current one; replacing an already-live subscription also cancels the one it replaces instead of leaking it.
...
- **Fixed: one user leaving used to wipe everyone else out of the panel.** Firebase frames a single user's removal from `chat_presence/<roomId>` as a `patch` event at path `/` with just that one key set to `null` — a multi-location update meaning "touch only these keys" — not a full-snapshot replace (which is what a `put` at `/` means). `applyPresenceEvent` used to treat *any* event at `/` as a full wipe-and-replace, so one departure cleared every other online user out of local state; everyone else then only reappeared as their own next heartbeat re-added them individually — the "whole sidebar disappears, then trickles back in" bug. Fixed by branching on the event type at `/`: `put` still replaces wholesale, `patch` now merges only the listed keys. Same fix applied to `applyTypingEvent` (C-Mail's typing indicator has the identical shape/bug). See `docs/00-api-backlog.md` and `TestHTTPSubscribeRoomPresence_RootPatchMergesInsteadOfReplacing` in `internal/api/client_test.go`.
...
| `internal/ui/screens/chatrooms_test.go` | Tests for `sortRoomUsers`, `panelWidths`, and presence message stale-guards (including stale/duplicate-subscription guards) |
| `internal/ui/screens/reconnect_test.go` | Reconnect-retry tests shared by C-Mail and CIRC, including the CIRC presence stream's reconnect sequence |
| `internal/ui/screens/render.go` | `renderCircMessages` (IRC-style format, no admin tag); `renderActionLine`, `renderSystemNotice` |
| `internal/ui/screens/render_test.go` | Rendering tests incl. inline-markdown-in-CIRC coverage, the `/me`-asterisk-body regression test, and the own-username-highlight style-continuity regression test |
| `internal/ui/markdown/renderer.go` | `Render`/`FirstLine` (full GFM, block-level, used by Feed/PostDetail/Journal/Guilds/Topics/C-Mail) and `RenderInline` (CIRC — paragraph-only block parser + CommonMark inline set + bare-URL linkify, no reflow; `highlightUser` param bolds the viewer's own name at the same raw-text rendering pass) |
| `internal/ui/markdown/renderer_test.go` | Tests for `Render`/`FirstLine`/`RenderInline`, incl. `RenderInline`'s no-block-misfire/no-reflow guards and the own-username-highlight style-continuity regression test |
