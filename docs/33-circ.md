...
- **`@mention` completion**: typing `@` starts an in-progress mention token (start-of-input or preceded by whitespace, so `user@host` never triggers); pressing `Tab` completes it in place to the first online user whose name matches the typed prefix (`@` alone offers everyone online, in the panel's admins-first/alphabetical order). Repeated `Tab` presses cycle to the next match in place — classic shell-completion style, not a dropdown — wrapping back to the first after the last. Typing anything else (including a space) locks in whatever's currently there and ends the cycle. No trailing space is inserted so a completed name can be extended or re-cycled without extra edits. Matches come from `m.roomUsers` (the same live presence list backing the users panel), so only people currently online are offered. See `mentionQueryAt`/`matchMentionCandidates`/`spliceMention` in `chatrooms.go`. A dim ghost-text preview of the top match (what pressing `Tab` would complete to) renders inline right after the cursor, but only when the mention token reaches the very end of the input — with trailing text after it (`hey @al there`) there's no unambiguous place to draw a preview, so `Tab`-cycling still works there, just without this visual hint (`ChatroomsModel.mentionGhostText`).
...
| `Tab` | Cycle `@mention` completion from online users (detail mode, compose input) |
