...
- **`@mention` completion**: typing `@` starts an in-progress mention token (start-of-input or preceded by whitespace, so `user@host` never triggers). `Tab` previews the first online user whose name matches the typed prefix (`@` alone previews everyone online, in the panel's admins-first/alphabetical order) as dim ghost text rendered immediately after the cursor — nothing is inserted into the real message yet, and the cursor never moves. Repeated `Tab` presses cycle which candidate is previewed, still without touching the real text. Typing a **space** commits whatever's currently previewed as real text, then inserts the space after it; typing anything else instead just clears the preview, leaving exactly what you actually typed untouched. Matches come from `m.roomUsers` (the same live presence list backing the users panel), so only people currently online are offered. The preview only renders when the mention token reaches the very end of the input — with trailing text after it (`hey @al there`) there's no unambiguous place to draw it, so `Tab`/space still work there, just without the visual hint. See `mentionCycle`, `mentionQueryAt`/`matchMentionCandidates`/`spliceMention`, and `ChatroomsModel.mentionGhostText` in `chatrooms.go`.
