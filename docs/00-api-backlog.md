...
Tracks gaps between the cyberspace.online API (v0.8.11) and what is currently implemented in the TUI client.
...
|---|---|---|---|---|
| `/v1/follows` | GET | **Open** | Response does not include `followerUsername` or `followedUsername`. Confirmed still missing in v0.8.7 (re-tested live via `apifetch` 2026-09-14 — every record has only `followId`/`followedId`/`followerId`/`createdAt`). The profile Following/Followers tabs fall back to showing a truncated user ID; profile navigation from those tabs is disabled until the API returns usernames. The Globe screen (feature 55) briefly had follow-network markers built on this endpoint; they never populated (no by-ID fallback for resolving a location) and were removed rather than kept as permanently-broken UI — Globe now plots guild members only, which don't have this problem since `GET /v1/guilds/:slug/members` does return `username`. | 2026-04-17 |
...
| `/v1/notifications` vs `/v1/notifications/unread-count` | GET | **Resolved (server-side, v0.8.11)** | The two endpoints used to disagree whenever muted/blocked/switched-off notification types piled up: `unread-count` counted them, the list (rightly) excluded them. First seen as a small skew (`{"count": 5}` vs. 3 visible items) on 2026-08-03 and accepted as documented/by-design at the time. Recurred at a much larger, user-visible scale on 2026-09-22 — an account with 100+ unread of a disabled type saw the TUI badge (and, presumably, any other client trusting the same endpoint) show "99+" while only 1-2 notifications were actually actionable, confirmed directly with the affected user. Client-side workaround shipped first (badge derived from paginating `GET /v1/notifications?read=false` instead of trusting the count endpoint). The API was then fixed properly in v0.8.11 — `docs/00-latest-api-reference.md` now states switched-off notifications never affect `exact`, with the exact example `{ "count": 2, "exact": true }` for 2 visible unread plus a muted pile. Confirmed via a live re-fetch/diff of `docs.md` (v0.8.10 → v0.8.11) on 2026-09-22; TUI reverted to calling `GetUnreadNotificationCount()` directly. | 2026-08-03 |
...
| `/docs.md` | GET | **Resolved — client updated same day** | `docs.md` now reports v0.8.10 live (previous snapshot on file was v0.8.9, one version behind — that jump wasn't separately logged here). Real content changes: `isSupporter` on `GET /v1/users/me`/`GET /v1/users/:username` (already implemented, feature 48); 4 new notification types (`guild_chat_message`, `moderator_permissions_changed`, `edit_access_granted`/`edit_access_removed`, `gift_received`/`gift_sent`) — **client updated same day**, see Notifications v0.8.10 row below; `GET /v1/cmail` conversations now flag a departed `otherUser` with `deleted: true` — **client updated same day**, see C-Mail section below. | 2026-09-16 |
| `/docs.md` | GET | **Resolved — client updated same day** | `docs.md` now reports v0.8.11 live. Only content change: `GET /v1/notifications/unread-count`'s docs now state that switched-off notification types never affect `exact` (example given: `{ "count": 2, "exact": true }` with 2 visible unread and a muted pile) — fixes the count/list skew logged in the `/v1/notifications` vs `/v1/notifications/unread-count` row above. `docs/00-latest-api-reference.md` re-fetched and diffed (only the version header and this one paragraph changed); the TUI's client-side pagination workaround for the same bug was reverted the same day, back to calling `GetUnreadNotificationCount()` directly. | 2026-09-22 |
...
| `otherUser.deleted` on `GET /v1/cmail` (v0.8.10) | A conversation partner who has deleted their account is flagged `deleted: true`. `model.User.Deleted` parses it; the client skips the doomed profile fetch and shows "(deleted)" in the conversation list and detail header instead of failing `p`/`ctrl+p` navigation. | **Done** — 2026-09-16 |
...
| `GET /v1/notifications` documented as excluding muted/blocked/off types | Verified non-issue: `fetchPage` (`internal/api/client.go`) and both `NotificationsModel.SetNotifs`/`AppendNotifs` (`internal/ui/screens/notifications.go`) already key pagination off `cursor == ""`, never off page length vs `limit` — the client was already correct, this row is closed as a documentation-only clarification. Explains (and supersedes) the count/list skew logged 2026-08-03 above. Note: the count/list skew this row closed as "expected" turned out to matter after all at scale — see the `/v1/notifications` vs `/v1/notifications/unread-count` row up in Known API Issues, resolved server-side in v0.8.11. | **Done (verified, no code change)** — 2026-08-13 |

### Notifications (new in v0.8.10)

| Area | Description | Priority |
|---|---|---|
| 4 new notification types | `guild_chat_message`, `moderator_permissions_changed`, `edit_access_granted`/`edit_access_removed`, `gift_received`/`gift_sent` — `notifSummary`/`notifIcon` cases and category-filter grouping added in `notifications.go`. Unverified against a live payload (all rare/admin- or feature-gated triggers). | **Done** — 2026-09-16 |
...
| `/mute` / `/unmute` — mixed-case username | Client-side bug, not a server bug: the server lowercases the `/unmute` argument before an exact-match removal against the stored mute entry, but cyber-tui sent `/mute`'s argument verbatim. A mixed-case `/mute JM2C` got stored as-is and could never be removed via `/unmute` — a permanently stuck mute. Fixed by lowercasing the username client-side before sending either command (`internal/ui/screens/chatrooms.go`), matching the server's lowercase-username convention. | 2026-09-16 |
