...
| `GET /v1/posts/:id`, `GET /v1/posts`, `GET /v1/posts/:postId/replies` | GET | **Unconfirmed — needs live check** | The edit endpoints' docs say "the entry then carries an `editedAt` timestamp," which the client (feature 43) reads as a persisted field that comes back on subsequent `GET`s to any viewer — `wirePost`/`wireReply` now parse an optional `editedAt`. But the docs' example `GET` response JSON blocks were never regenerated to literally include it, so this is inferred from prose, not confirmed against a live response. Verify via `apifetch GET /v1/posts/:id` on a freshly-edited post; if `editedAt` is actually absent from `GET` responses (i.e. only ever returned to the editor, or not returned at all), the `(edited)` badge premise in `docs/43-edit-post.md` needs revisiting — right now no UI renders the badge yet, so nothing user-facing depends on this being correct today. | 2026-08-12 |
