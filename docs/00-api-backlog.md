...
|---|---|---|---|---|
| `/v1/follows` | GET | **Open** | Response does not include `followerUsername` or `followedUsername`. Confirmed still missing in v0.8.7 (re-tested live via `apifetch` 2026-09-14 — every record has only `followId`/`followedId`/`followerId`/`createdAt`). The profile Following/Followers tabs fall back to showing a truncated user ID; profile navigation from those tabs is disabled until the API returns usernames. The Globe screen's follow markers (feature 55) have no such fallback and never populate at all: unlike the profile tabs, resolving a location requires a username (`GET /v1/users/:username` is the only profile-lookup endpoint, no by-ID equivalent), so `loadGlobeFollowsCmd` silently drops every follow record. Guild markers are unaffected — `GET /v1/guilds/:slug/members` does return `username`. | 2026-04-17 |
