...
| `GET /v1/notifications` vs `/v1/notifications/unread-count` | GET | **Open — notification exists server-side but is unreachable via REST** | The web UI shows a genuine unread "System" notification about the too-soon-post-to-journal conversion above, and `unread-count` agrees (`{"count": 1}`), but `GET /v1/notifications` never returns it — confirmed with no filter, `read=false`, and a 50-item unfiltered page, repeated minutes apart. This is stronger than the already-documented count/list caching skew (2026-08-03 entry above): that one is a timing/threshold discrepancy; this is total, persistent exclusion of one specific notification (and presumably its whole undocumented type — likely something in the "system_*" family not in the documented enum, since `type=system_ban` still 500s per the known `type` filter bug) from every list response. Consequence: no REST client, the TUI included, can ever fetch or display this notification — it isn't a TUI rendering gap, the API itself won't hand it over outside the unread count and whatever internal path the website uses. No client-side fix possible. | 2026-08-12 |
...
| Area | Description | Priority |
|---|---|---|
| 8 new notification types | `graffiti_mention`, `moderator_granted`/`moderator_removed`, `api_access_granted`/`api_access_removed`, `system_ban_lifted`, `post_cooldown`, `rate_limit_warning`. `notifSummary`/`notifIcon` (`notifications.go`) don't have cases for these yet — they'd fall through to whatever the default/unknown-type rendering is. | Medium — cosmetic only until one is actually observed live |
| `actorId`/`actorUsername` optional, or literal `"system"` | Now documented explicitly for account-level notifications (`system_ban`, `post_cooldown`, `rate_limit_warning`, `system_ban_lifted`). Client should confirm it doesn't try to build a profile link/navigation off `"system"` as if it were a real username. | Medium |
| `unread-count` gains `exact: false` above 100 unread | Badge should render "99+" rather than a specific number once `exact` is `false`, per the new documented cap. Not currently checked client-side. | Low — cosmetic, only matters at >100 unread |
| `read-all` gains `hasMore` | `POST /v1/notifications/read-all` caps at 5,000 marked per call; a client with a very large unread backlog needs to loop while `hasMore` is `true`. Not currently looped client-side (`MarkAllNotificationsRead` or equivalent). | Low — only matters at extreme unread counts |
| `GET /v1/notifications` documented as excluding muted/blocked/off types | Explains (and likely supersedes) the count/list skew logged 2026-08-03 above — a short page is now expected behavior, not a bug. Client pagination should key off `cursor` being non-null, not off getting a full page of `limit` items. Worth auditing `GetNotifications` callers for a stop-when-short assumption. | Medium — correctness risk if any caller assumes a full page means more pages exist |
