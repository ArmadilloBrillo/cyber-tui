{
  "data": [
    {
      "id": "8mzQJxC9WkIMmN38jmGX",
      "type": "new_post_friend",
      "actorId": "WWLU5qqbUvSXEdGKU7fbWXRfkya2",
      "actorUsername": "era",
      "targetId": "ZJXYZ2EeP27Eb5iWJHEU",
      "targetType": "post",
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "read": false,
      "createdAt": "2026-04-17T01:38:58.898Z"
    },
    {
      "id": "c4N3N7qNG7x6qSlhVHQL",
      "type": "new_post_friend",
      "actorId": "JcUWvxNJ4cObXqckegmc6UtyIJS2",
      "actorUsername": "7spires",
      "targetId": "5OmvmzSL4snAfZLiXNKq",
      "targetType": "post",
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "read": false,
      "createdAt": "2026-04-17T00:15:22.051Z"
    },
    {
      "id": "qyIri9l8YdbikyMdSRKh",
      "type": "thread_reply",
      "actorId": "EqfN525IIpaVVZS01SiWtMRYMgh2",
      "actorUsername": "twist",
      "targetId": "0QvhOjNaQkoKcw0Gai1p",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "landon",
        "replyId": "A2kSSnHIex1ii8Gpkgui"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "read": false,
      "createdAt": "2026-04-16T23:05:39.045Z"
    },
    {
      "id": "cRuu7siNW0B8EylpRv3b",
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "type": "bookmark",
      "actorId": "gdu7QLtd2mdudlhMfYWYbngqfMZ2",
      "actorUsername": "dang",
      "targetId": "lKfvsKxQag8FAiptlqUk",
      "targetType": "post",
      "metadata": {
        "authorUsername": "ragnar"
      },
      "read": false,
      "createdAt": "2026-04-16T22:15:30.028Z"
    },
    {
      "id": "OrGZWxZ0MgbuzV46lskG",
      "type": "thread_reply",
      "actorId": "SmZeudWzoRXzOKmDyRmUekeomzf2",
      "actorUsername": "landon",
      "targetId": "0QvhOjNaQkoKcw0Gai1p",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "landon",
        "replyId": "JB6qYtLs70Rq98clqFim"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "read": false,
      "createdAt": "2026-04-16T22:13:58.086Z"
    },
    {
      "id": "qV4DLpAZri2i7avEnyR8",
      "type": "thread_reply",
      "actorId": "SmZeudWzoRXzOKmDyRmUekeomzf2",
      "actorUsername": "landon",
      "targetId": "0QvhOjNaQkoKcw0Gai1p",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "landon",
        "replyId": "lHLOWjmKUjQmLlP7ZCBm"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "read": false,
      "createdAt": "2026-04-16T21:59:11.445Z"
    },
    {
      "id": "fi4w3OdYgP236fe3Mltg",
      "type": "thread_reply",
      "actorId": "SmZeudWzoRXzOKmDyRmUekeomzf2",
      "actorUsername": "landon",
      "targetId": "0QvhOjNaQkoKcw0Gai1p",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "landon",
        "replyId": "b42KVZDqXCZI36qiUw9Y"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "read": false,
      "createdAt": "2026-04-16T21:59:04.740Z"
    },
    {
      "id": "F8KG3DU1gmSUTgfqV1fC",
      "type": "guild_new_thread",
      "actorId": "x1fMIYq7mFUXfyJlOTfGbH1VAit1",
      "actorUsername": "the_g",
      "targetId": "Vf9s1FrzbYuKkD8zdTgW",
      "targetType": "post",
      "metadata": {
        "guildSlug": "technica",
        "guildName": "technica",
        "isGuildThread": true,
        "threadId": "Vf9s1FrzbYuKkD8zdTgW"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "createdAt": "2026-04-16T21:30:39.369Z",
      "read": true
    },
    {
      "id": "MTJGUAtUVww8dtRvwo9o",
      "type": "thread_reply",
      "actorId": "TAWyWtaxa7fA5TadYrUO0nWt4B72",
      "actorUsername": "bigredimpulse",
      "targetId": "8FoHw0qzMN35T9NecJ4D",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "genghis_khan",
        "replyId": "7YReP8o0sREqLYEVv9xp"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "createdAt": "2026-04-16T18:34:11.675Z",
      "read": true
    },
    {
      "id": "cXMFUaRg5mP5JAREljpd",
      "type": "thread_reply",
      "actorId": "iLFZ1bGdP1OPJGftmCI337MnbxX2",
      "actorUsername": "genghis_khan",
      "targetId": "v9FiTrLHLeFPR2VvklEz",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "7spires",
        "replyId": "gyOVj6FpOizITrPF4WJl"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "createdAt": "2026-04-16T18:02:44.931Z",
      "read": true
    },
    {
      "id": "yGauZmPfVTrIcudcglwL",
      "type": "thread_reply",
      "actorId": "VVvvhAvq6mMzD4QonjPIPtQwwYG3",
      "actorUsername": "aqwek",
      "targetId": "v9FiTrLHLeFPR2VvklEz",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "7spires",
        "replyId": "kB7ZYy6O8AwRI6IQc9Gt"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "createdAt": "2026-04-16T18:02:19.471Z",
      "read": true
    },
    {
      "id": "g3GxeN6MmRgTYeCfrWVa",
      "type": "new_post_following",
      "actorId": "iLFZ1bGdP1OPJGftmCI337MnbxX2",
      "actorUsername": "genghis_khan",
      "targetId": "VlH4ErHnYNeaFe0L6rJy",
      "targetType": "post",
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "createdAt": "2026-04-16T18:00:54.196Z",
      "read": true
    },
    {
      "id": "IdGGeCvMbP7bPCl58bvO",
      "type": "thread_reply",
      "actorId": "YpInNcMwdZVtZcLRU7oFxFzGxYz2",
      "actorUsername": "ninso",
      "targetId": "Idxzw831RTfOm0klyJZp",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "default_hacktip",
        "replyId": "rF3o5D9lEJJOnsUZ4Mtp"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "createdAt": "2026-04-16T17:12:57.350Z",
      "read": true
    },
    {
      "id": "7sdACn39D1yPHpG5zFNE",
      "type": "thread_reply",
      "actorId": "JcUWvxNJ4cObXqckegmc6UtyIJS2",
      "actorUsername": "7spires",
      "targetId": "0QvhOjNaQkoKcw0Gai1p",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "landon",
        "replyId": "MT5bwKZGnXrv6nCEkKbH"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "createdAt": "2026-04-16T16:15:01.322Z",
      "read": true
    },
    {
      "id": "gTjVfDDqIjEQ6LVL39lU",
      "type": "thread_reply",
      "actorId": "tD60oZSvBOPDjIbl9ax1aMkEy2i2",
      "actorUsername": "spacecowboy",
      "targetId": "0QvhOjNaQkoKcw0Gai1p",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "landon",
        "replyId": "zSIX4s36MwFusazWDFzS"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "createdAt": "2026-04-16T16:13:07.526Z",
      "read": true
    },
    {
      "id": "A0t3cmW3EF1to0gfjEbm",
      "type": "thread_reply",
      "actorId": "e6Ue4sxVA6bEzdImNExL7xeMaC13",
      "actorUsername": "itisi",
      "targetId": "Idxzw831RTfOm0klyJZp",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "default_hacktip",
        "replyId": "xQ6CtBppViiX2zLcjTEU"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "createdAt": "2026-04-16T16:12:10.401Z",
      "read": true
    },
    {
      "id": "BgeRv7FOiTD39UeQm99x",
      "type": "thread_reply",
      "actorId": "e6Ue4sxVA6bEzdImNExL7xeMaC13",
      "actorUsername": "itisi",
      "targetId": "0QvhOjNaQkoKcw0Gai1p",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "landon",
        "replyId": "9uHo0WdAkHT25M0eUBD8"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "createdAt": "2026-04-16T16:07:58.282Z",
      "read": true
    },
    {
      "id": "M2yC7c3SoOdtBjx9udyR",
      "type": "thread_reply",
      "actorId": "heJtywb2KFPwLI31f0koxV2UaRh2",
      "actorUsername": "riverfloater",
      "targetId": "0QvhOjNaQkoKcw0Gai1p",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "landon",
        "replyId": "gxeZYDcU6eJnN8HJ3dPV"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "createdAt": "2026-04-16T16:04:45.759Z",
      "read": true
    },
    {
      "id": "PDRJgrML0OlkyUHpv1KZ",
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "type": "reply",
      "actorId": "5vzCvkEyQQS0aje32X6TP0mfWpm2",
      "actorUsername": "mikozdov",
      "targetId": "BgjecEfyBZfdyvIifkn1",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "ragnar",
        "replyId": "csJeWPiIHtJYwurvzqUd",
        "parentReplyId": "kmgZeu5zJueasoLMhF0A"
      },
      "createdAt": "2026-04-16T15:52:21.039Z",
      "read": true
    },
    {
      "id": "kDhC5MfXRtGOvxPK8LyW",
      "type": "thread_reply",
      "actorId": "5vzCvkEyQQS0aje32X6TP0mfWpm2",
      "actorUsername": "mikozdov",
      "targetId": "BgjecEfyBZfdyvIifkn1",
      "targetType": "reply",
      "metadata": {
        "authorUsername": "cindospen",
        "replyId": "cw7GxLp6hHxNQ52VVG5m"
      },
      "userId": "5dfFKBZ1t1VZRH4PKgeL0Vw7sTi2",
      "createdAt": "2026-04-16T15:50:52.979Z",
      "read": true
    }
  ],
  "cursor": "kDhC5MfXRtGOvxPK8LyW"
}
