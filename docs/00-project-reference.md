...
| **Inline/fullscreen images on WezTerm/Windows: known, accepted limitation** | Do not render — a fragment appears in the wrong screen position instead. Root cause: `injectInlineImages` batches each image's cursor-position (CUP) escape with the image OSC sequence into one write, and Bubble Tea's renderer (`standard_renderer.go`, vendored) always appends its own trailing cursor-position CSI after all frame content in the same syscall write — ConPTY (Windows' only pty layer, unavoidable for a native Win32 console app) is known to reorder an OSC relative to a CSI that follows it in the same write (`microsoft/terminal#17314`), so the image renders at whatever position that later CSI set instead of its own. Confirmed by isolated replay (the exact same captured bytes render correctly when written alone, outside Bubble Tea). Not fixable within cyber-tui without bypassing Bubble Tea's renderer for image writes entirely — judged not worth the architectural risk for a Windows-only ConPTY bug; not planned. See `docs/plan-inline-images-improvements.md` §9 for the full investigation. The unrelated fixes found along the way (WezTerm's own scroll-on-last-line bug, and images not being downscaled to their display size before encoding) are kept — both genuinely correct regardless of this outcome |
