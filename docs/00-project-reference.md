...
- **Desktop notifications** (opt-in, `desktopNotifications` local config): when the activity unread-count rises (`unreadCountMsg`) or new C-Mail lands while backgrounded (`handleCMail` default branch), emits an OSC 9 escape to `os.Stdout` via `desktopNotifyCmd` (`desktopnotify.go`), gated by `shouldDesktopNotify()` — enabled, not an SSH/ephemeral session, and the terminal not known-focused. Terminal focus is tracked from `tea.FocusMsg`/`tea.BlurMsg` (`tea.WithReportFocus()` in `main.go`), handled in `updateInner`. One-shot baseline guards (`notifCountBaselined`, `cmailUnreadBaselined`) suppress the login backlog. See `docs/53-desktop-notifications.md`
