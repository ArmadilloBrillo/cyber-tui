...
- **Slash commands:** normal commands (`/me`, `/dice`, etc.) are expanded server-side and need no client handling; `/help` posts nothing, so `SendMessage`'s reply text is routed through app.go's `cmailCommandReplyMsg` into `AppendSystemMessage`, which injects a local-only `model.Message{IsSystem: true}` rendered via `renderSystemNotice`
...
- **Slash commands:** normal commands (`/me`, `/dice`, etc.) are expanded server-side and need no client handling; `/help` posts nothing, so `SendRoomMessage`'s reply text is routed through app.go's `roomCommandReplyMsg` into `AppendSystemMessage`, which injects a local-only `model.Message{IsSystem: true}` rendered via `renderSystemNotice` (shared with `cmail.go`)
