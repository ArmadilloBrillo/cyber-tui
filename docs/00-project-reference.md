...
| `RoomUser` | Presence entry for a cIRC room (userID, username, isChatAdmin, lastSeen, lastActivity — nil means always active) |
...
| `Guild` | Guild community (ID, name, slug, icon, bio, memberCount, apprenticeCount, founderUsername, createdAt, isMember, role, link, linkText, profilePictureUrl) |
| `GuildMember` | Guild membership record (membershipID, guildID, guildSlug, userID, username, role, joinedAt, displayName, profilePictureUrl); role is founder/member/apprentice |
| `GuildMembership` | Entry from a user's guilds list (guildID, slug, name, icon, profilePictureUrl, role, joinedAt) — badge guild first, then apprenticeships |
...
| Guilds | `GetGuilds(cursor)`, `GetGuild(slug)`, `GetGuildPosts(slug, cursor)`, `CreateGuildPost(slug, content, title, postSlug, topics)`, `GetGuildMembers(slug, cursor)`, `GetUserGuilds(username)`, `JoinGuild(slug) (role, error)`, `LeaveGuild(slug)`, `PromoteGuild(slug) (role, error)` |
...
| Direct Messages | `GetConversations()`, `GetMessages(convID, limit)`, `SendMessage(convID, body)`, `StartConversation(recipientUsername)`, `MarkCMailRead(convID)`, `SubscribeDMs(ctx, convID) <-chan model.Message`, `AnnounceTyping(convID)`, `ClearTyping(convID)`, `SubscribeDMTyping(ctx, convID, staleAfterMs) <-chan []model.TypingUser`, `SubscribeUserConversations(ctx, uid, initial) <-chan []model.Conversation` |
| Chatrooms | `GetRooms()`, `GetRoomMessages(roomID, limit, before)`, `SendRoomMessage(roomID, body)`, `MarkRoomRead(roomID)`, `SubscribeRoom(ctx, roomID) <-chan model.Message`, `GetRoomUsers(roomID)`, `AnnouncePresence(roomID, lastActivity) (heartbeatMs, staleAfterMs, idleAfterMs, err)`, `LeaveRoomPresence(roomID)`, `SubscribeRoomPresence(ctx, roomID, staleAfterMs) <-chan []model.RoomUser` |
...
- ESC emits `BackToFeedMsg` → if `App.postDetailStack` is non-empty, App pops it and restores that snapshot (post, loaded replies, scroll position — no refetch) instead; only once the stack is empty does App return to `postDetailReturn` (whichever of the 7 origins — Feed, Profile, Bookmarks, Guilds, Topics, Notifications, Search — opened the post) and close it (`PostDetailModel.Close()`)
- **Nested post-to-post links**: a post's content can link to another post (`routeURL`/`urlPostLoadedMsg` in `app.go`); opening one while already in PostDetail pushes the current `PostDetailModel` onto `postDetailStack` rather than overwriting `postDetailReturn` with `screenPostDetail` itself (the bug that used to leave ESC doing nothing after following such a link). The origin-tab escape hatch below also clears `postDetailStack` so a stale nested session can't leak into a later, unrelated post visit.
...
- Same InlineImages gate also renders a **supporter badge icon** inline next to the username line and on the Info tab's Supporter row — see `docs/48-badge-icons.md`. Off/unsupported, the Supporter row falls back to its pre-existing text (`"yes (pi)"`). Guild badges were attempted in the same feature and reverted (`GuildIcon` uses an unresolvable icon-naming scheme, not the supporter-icon convention) — the Guild row stays plain text
...
- When InlineImages is gated on, the detail header's `"@other"` also shows the other participant's **supporter badge icon** — detail-header-only, same as the typing indicator; never the conversation list card, never per-message. Conversation data alone never carries `SupporterIcon` (thin `wireCMailOtherUser` shape), so opening a conversation additionally fetches the participant's full profile (`OtherProfileFetchTarget`/`loadCMailOtherProfileCmd`/`SetOtherProfile`, cached by username, never re-fetched) — the badge silently stays absent until that resolves. Guild badge is not attempted (`GuildIcon` uses an unresolvable icon-naming scheme). See `docs/48-badge-icons.md`.
...
Exported background-tab helpers: `IsDMStreamMsg(msg) bool` (identifies the internal types above so `App` can route them regardless of active screen), `SetFocused(bool)`, `HasLiveConv() bool`, `ComposeEmpty() bool` — no standalone `UnreadCount() int` the way Chatrooms has one; the badge stays `TotalUnread()` (sum across all conversations), deliberately not CIRC's single-room counter
...
- Thread feed shows a membership hint bar (`J` to join, `L` to leave, `P` to promote an apprenticeship to badge) with y/n confirmation; `GetGuild` is fetched alongside the thread list to populate membership state (`role` is `""`, `"member"`, `"apprentice"`, or `"founder"`)
- Member list is cursor-paginated oldest-joined first, mixing members and apprentices (role icon `◆`/`•`/`◇`); `enter` navigates to the member's profile, `esc` returns to the thread feed
- Thread feed now supports inline images the same as Feed (`RenderPost(..., inlineImagesEnabled)`, `VisibleInlineImages()`)
- Profile's Info tab shows an "Apprentice" row listing the viewed user's apprenticeships (fetched via `GetUserGuilds`, non-fatal on error)
- Guild list icons stay text-only (`◆`/★/☆, `guildIcon()`/`isEmojiIcon()`, unchanged from before feature 48) — a real inline-image badge was attempted and reverted, since `Guild.Icon` values are confirmed live to be Unicode/CLDR emoji character names or an unidentified `dinkie-icons:` set, not a resolvable icon-library code. See `docs/48-badge-icons.md` and `docs/00-api-backlog.md`.

Key types: `GuildsModel`, `LoadMoreGuildsMsg`, `LoadGuildPostsMsg`, `LoadMoreGuildPostsMsg`, `LoadGuildMembersMsg`, `LoadMoreGuildMembersMsg`, `JoinGuildMsg`, `LeaveGuildMsg`, `PromoteGuildMsg`  
...
Key accessors: `IsBrowsingGuild()`, `IsBrowsingMembers()`, `IsDetailLoaded()`, `GuildDetail()`, `IsConfirmingJoin()`, `IsConfirmingLeave()`, `IsConfirmingPromote()`
...
`internal/ui/layout.go`, so the numeric aliases and leader chords can never disagree
about what a given key does.
...
**Search is hidden from the tab bar and from `←`/`→` cycling** — `menuTabs`' `search` entry has
...
| `ctrl+←` / `ctrl+→` | Same as `←`/`→`, but reaches the handler even while a compose input is focused |
...
accounts only) for attaching a YouTube track: URL, Artist (max 100 chars),
Title (max 150), Genre (max 50, lowercased on read). All three of
Artist/Title/Genre are required — matches the API's documented shape for a
post/reply audio attachment. Artist/Title auto-fill best-effort from the URL
via `internal/youtube`'s oEmbed lookup; Genre is always manual (oEmbed has no
genre field). Opens from CIRC's composer, Feed's new-post panel, and Post
Detail's edit panel/reply compose — not C-Mail
(`docs/49-song-attach.md`, `docs/50-post-reply-attachments.md`).
...
| `enter` on genre, or `ctrl+s` | Submit — in CIRC, hands the built `/song ...` command to the composer (still requires `enter` there to send); on a post/reply target, sets a native audio attachment directly |
...
| `github.com/srwiley/oksvg` | v0.0.0-20221011165216-be6e8873101c | SVG parsing for badge icons (`imgview.FetchBadgeIcon`) — see `docs/48-badge-icons.md` |
| `github.com/srwiley/rasterx` | v0.0.0-20220730225603-2ab79fcdd4ef | SVG rasterization paired with oksvg above |
