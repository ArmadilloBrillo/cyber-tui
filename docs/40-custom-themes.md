...
- [x] Markdown-wrapped post fields bug fixed (`ParsePost` strips markdown decoration per line via `stripLineDecoration`/`lineDecorationCutset` — backticks, blockquotes, bold/italic, bullets, in any combination), covered by a unit test on the helper plus regression tests using two real posted formats (backtick-wrapped, blockquote-wrapped)
